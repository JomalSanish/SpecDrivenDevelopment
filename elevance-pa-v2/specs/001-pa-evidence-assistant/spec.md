...
- What happens when the completeness pipeline encounters an LLM timeout or failure? → The case remains in "Processing" state. After a configurable retry window, if the pipeline still fails, the case is flagged "Pipeline Error" and an admin/nurse can view it — it never silently disappears.
...
- What happens if the AI extraction of case fields returns no confident results at all? → All fields remain blank; the form is entirely manual. Submission is still possible as long as the required fields (member ID, policy selection) are filled manually.
...
- **FR-005**: The system MUST hash stored passwords using a memory-hard algorithm. Plaintext passwords MUST never be stored or logged.
...
- **FR-011**: Upon document upload, the system MUST attempt native text extraction per page first. For pages that yield near-empty text, the system MUST apply GPU-accelerated OCR as a fallback.
- **FR-012**: Each extracted text chunk MUST carry provenance metadata indicating whether its text came from native extraction or OCR.
...
- **FR-017**: The case list MUST display current status (e.g., Processing, Ready for Review, Accepted, Rejected) and be searchable and filterable by status, service type, and submission date.
...
- **FR-023**: For mixed requirements, the system MUST combine dense and sparse (BM25) retrieval results using Reciprocal Rank Fusion. If the top-ranked chunk has strong semantic relevance but zero keyword corroboration on an identifier-bearing field, the requirement verdict MUST be capped at "Unclear."
...
- **FR-032**: The lock MUST auto-release after 30 consecutive minutes of inactivity by the locking nurse. The case then becomes available to any nurse.
...
- **FR-038**: If an admin has re-queued a previously decided case, it MUST appear in the nurse queue with an **"Admin Edit"** badge and the admin's comment visible at the top of the case detail view.

#### Policy Management

- **FR-040**: Policy upload (including the ability to save or finalize a policy) MUST be restricted to the admin role. Intake and nurse users MUST be able to view the policy list and read requirement checklists in a read-only mode.
...
- **FR-044**: Re-uploading a policy using a name that already exists in the system MUST overwrite the existing policy and its requirement checklist. No versioning or history of the prior policy MUST be retained in the primary data.
...
- **Case**: A single prior authorization request. Has member ID, provider, CPT/HCPCS code, ICD-10 code, requested service/procedure, requested date, associated policy, current status, queue assignment, lock state (locked by, locked at), decision (decided by, decided at, decision type), and a history of status transitions.
...
- A SLA escalation mechanism (auto-escalating cases that sit in the review queue beyond a time threshold) is part of the background infrastructure inherited from the v1 architecture (SLA service) and is included in scope.
